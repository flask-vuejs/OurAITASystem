<template>
  <div class="project-list">
    <div class="project-item" v-for="(item, index) in zxxms" :key="index">
      <div @click.prevent="to_detail(item.id)">
        ...
        <div class="project-stats">
          <span><i class="fas fa-users"></i>负责人： {{ item.principal_investigator }}</span>
...
async function getZxxms() {
  try{
    const res:any=await axios.get<any>('http://127.0.0.1:5000/zxxm/list')
    if(res['data']['code']==200){
      zxxms.value=res['data']['data']
      //console.log(zxxms.value)
    }
  }catch(e){
    console.log(e)
  }
